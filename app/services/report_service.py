...
class ReportGenerationService:
    ...
    def generate_santander_reaching_consumers(self, destination_country_code, countries_config, login_required=True):
        """Generates the Reaching the Consumer data by scraping Santander Trade."""
    ...
    def generate_santander_distributing_product(self, destination_country_code, countries_config, login_required=True):
        """Generates the Distributing a Product data by scraping Santander Trade."""
        # Get country name from code
        destination_country_name = get_country_name_from_code(destination_country_code, countries_config)
        if not destination_country_name:
            return {"error": f"Country not found for code: {destination_country_code}"}
    ...
    def generate_santander_identify_suppliers(self, destination_country_code, countries_config, login_required=True):
        """Generates the Identify Suppliers data by scraping Santander Trade."""
        # Get country name from code
        destination_country_name = get_country_name_from_code(destination_country_code, countries_config)
        if not destination_country_name:
            return {"error": f"Country not found for code: {destination_country_code}"}
        ...
        try:
            ...
            print(f"Scraping Identify Suppliers for {formatted_country_name}...")
            scraped_data = scrape_santander_identify_suppliers(self.driver, formatted_country_name)
            return scraped_data
        except Exception as e:
    ...
    def generate_santander_business_directories(self, industry_name, destination_country_name, login_required=True):
        """Generates the Business Directories data by scraping Santander Trade."""
        try:
            if not self.driver:
                print("Driver not available. Skipping scraping.")
                return {"error": "Driver not available"}
            ...
            print(f"Scraping Business Directories for industry: {industry_name}, country: {destination_country_name}...")
            scraped_data = scrape_santander_business_directories(self.driver, industry_name, destination_country_name)
            
            # Add country name to the returned data for proper display (if not already there)
            if not scraped_data.get('error'):
                scraped_data['country_name'] = destination_country_name
                scraped_data['industry_name'] = industry_name
            
            return scraped_data
        except Exception as e:
            print(f"An error occurred during Business Directories scraping: {e}")
            return {"error": f"Error during Business Directories scraping: {str(e)}"}

    def generate_santander_professional_associations(self, industry_name, destination_country_name, login_required=True):
        """Generates the Professional Associations data by scraping Santander Trade."""
        try:
            if not self.driver:
                print("Driver not available. Skipping scraping.")
                return {"error": "Driver not available"}
            ...
            print(f"Scraping Professional Associations for industry: {industry_name}, country: {destination_country_name}...")
            scraped_data = scrape_santander_professional_associations(self.driver, industry_name, destination_country_name)
            ...
            print(f"An error occurred during Professional Associations scraping: {e}")
            return {"error": f"Error during Professional Associations scraping: {str(e)}"}

    def generate_santander_online_marketplaces(self, industry_name, destination_country_name, login_required=True):
        """Generates the Online Marketplaces data by scraping Santander Trade."""
        try:
            if not self.driver:
                print("Driver not available. Skipping scraping.")
                return {"error": "Driver not available"}
            ...
            print(f"Scraping Online Marketplaces for industry: {industry_name}, country: {destination_country_name}...")
            scraped_data = scrape_santander_online_marketplaces(self.driver, industry_name, destination_country_name)
